...
def test_transaction(lock_path, transaction, type):
    ...
    with transaction(lock, enter_fn, exit_fn):
        assert vals['acquired_%s' % type]
        assert not vals['released_%s' % type]
...
def test_transaction_with_context_manager(lock_path, transaction, type):
    ...
    with transaction(lock, TestContextManager, exit_fn):
        pass
    ...
    vals.clear()
    with transaction(lock, TestContextManager):
        pass
...
def test_nested_write_transaction(lock_path):
    ...
    # write/write
    with lk.WriteTransaction(lock, None, write):
...
def test_nested_reads(lock_path):
    ...
    vals.clear()
    assert vals['read'] == 0
    with lk.ReadTransaction(lock, read):
        assert vals['read'] == 1
        with lk.ReadTransaction(lock, read):
            assert vals['read'] == 1
    ...
    vals.clear()
    assert vals['read'] == 0
    with lk.WriteTransaction(lock, read):
        assert vals['read'] == 1
        with lk.WriteTransaction(lock, read):
            assert vals['read'] == 1
